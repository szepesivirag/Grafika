...
GLuint load_texture(const char* filename) {
    ...
    // Az OpenGL alulról felfelé olvassa a textúrákat, a JPG/PNG viszont felülről lefelé.
    // Ez megfordítja nekünk a képet betöltéskor
    stbi_set_flip_vertically_on_load(false); 

    unsigned char* image = stbi_load(filename, &width, &height, &nrChannels, 4);
    ...
    if (image) {
        ...
        //csak ha nem találáná, legyen ellenőrizve.
    }
...
void init_entities(World* world) {
    ...
    init_object(&world->table, 120, 170);
	// Room
	world->room.ground = load_texture("textures//floor.jpg");
	world->room.back = load_texture("textures//window.jpg");
	world->room.front = load_texture("textures//window.jpg");
	world->room.left = load_texture("textures//banana.jpg");
	world->room.right = load_texture("textures//banana.jpg");
	world->room.top = load_texture("textures//ceiling.jpg");

	// Help
	world->helpTexture = load_texture("textures//help1.png");
}
